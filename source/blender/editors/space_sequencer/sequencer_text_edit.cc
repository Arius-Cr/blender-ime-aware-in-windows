...
static bool text_insert_utf8(TextVars *data, const char *buf, int buf_len)
{
  ...
  /* Maximum bytes that can be filled into `data->text`. */
  const int fillable_len = std::min(buf_len, max_str_len);

  /* Truncated string could contain invalid utf-8 sequence, thus ensure the length inserted is
   * always valid. */
  size_t valid_str_len;
  const int extra_offset = BLI_strnlen_utf8_ex(buf, fillable_len, &valid_str_len);
  ...
  std::memmove(cursor_addr + valid_str_len, cursor_addr, move_str_len);
  std::memcpy(cursor_addr, buf, valid_str_len);

  data->cursor_offset += extra_offset;

  if (valid_str_len != buf_len) {
    return false;
  }
...
static int ime_input_invoke(bContext *C, wmOperator *op, const wmEvent *event)
{
  ...
  if (event->type == WM_IME_COMPOSITE_START) {
     ...
     */
    WM_operator_name_call(C, "SEQUENCER_OT_ime_insert", WM_OP_INVOKE_REGION_WIN, nullptr, event);
  }
  ...
  if (event->type == WM_IME_COMPOSITE_EVENT) {
    ...
    if (ime_data->composite_len != 0) {
      ...
      bool all_insterd = text_insert_utf8(
          strip_data, ime_data->str_composite, ime_data->composite_len);
...
static int ime_insert_invoke(bContext *C, wmOperator * /*op*/, const wmEvent *event)
{
  ...
  if (event->type == WM_IME_COMPOSITE_EVENT) {
    ...
    if (ime_data->result_len) {
      ...
      text_insert_utf8(data, ime_data->str_result, ime_data->result_len);
